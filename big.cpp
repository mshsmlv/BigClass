...
Big Division(Big &e, Big &c, Big &remainder)
{
    ...
    for (j; (B_Length - m) <= j; j--) {
        ...
        if (DEBUG_MODE) {
            ...
            cout << "a.al[n-1]: " << hex << a.al[n - 1] << endl;
        ...
        while (1) {
            ...
            right_part = (static_cast<doubleBase>(b.al[j]) << sizeof(base) * 8) + static_cast<doubleBase>(b.al[j - 1]) -
                roof * static_cast<doubleBase>(a.al[n - 1]);
            if (right_part >> sizeof(base) * 8) {
                if (DEBUG_MODE)
                    cout << "переполнение правой части " << endl;
                //  std::cout << hex << right_part << endl << hex << left_part << endl;
        ...
        int k, l;
        new_num.ar = new_num.al + a.GetLength();
        if (DEBUG_MODE)
            cout << "new_num length " << new_num.GetLength() << endl;
        for (k = j, l = new_num.GetLength() - 1; l >= 0; l--, k--) {
            if (1) {
                cout << "k=" << k << endl;
        ...
        new_num.Compress();
        if (DEBUG_MODE)
            cout << "length new_num = " << new_num.GetLength() << endl;

        for (k = j, l = new_num.GetLength() - 1; l >= 0; l--, k--) {
            if (DEBUG_MODE)
                cout << "выписываем " << endl;
            b.al[k] = new_num.al[l];
        }
